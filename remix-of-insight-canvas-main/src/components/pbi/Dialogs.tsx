...
import { SALES, MEASURES, TABLES, formatNumber, type Row } from "@/lib/pbi/model";
...
export function PowerQueryDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const columns = def.fields.map((f) => f.name).filter((c) => !hidden.includes(c));

  const rows: Row[] = useMemo(() => {
    const data = [...SALES].slice(0, 400);
    if (sortCol) data.sort((a, b) => String(a[sortCol]).localeCompare(String(b[sortCol])));
  ...
  const RIBBON: Record<string, { label: string; run: () => void }[]> = {
    Home: [
      { label: "Refresh preview", run: () => addStep("Refreshed Preview") },
      { label: "Choose columns", run: () => addStep("Removed Other Columns") },
      {
    ...
    Transform: [
      ...
      { label: "Use first row as headers", run: () => addStep("Promoted Headers") },
      { label: "Transpose", run: () => addStep("Transposed Table") },
      { label: "Pivot column", run: () => addStep("Pivoted Column") },
      { label: "Unpivot columns", run: () => addStep("Unpivoted Columns") },
      { label: "Replace values", run: () => addStep("Replaced Value") },
      { label: "Split column", run: () => addStep("Split Column by Delimiter") },
    ],
    "Add Column": [
      { label: "Custom column", run: () => addStep("Added Custom") },
      { label: "Conditional column", run: () => addStep("Added Conditional Column") },
      { label: "Index column", run: () => addStep("Added Index") },
      { label: "Duplicate column", run: () => addStep("Duplicated Column") },
      { label: "Extract year", run: () => addStep("Inserted Year") },
    ],
    View: [
      { label: "Formula bar", run: () => toast.info("Formula bar toggled") },
      { label: "Advanced editor", run: () => toast.info("let Source = Sales in Source") },
      { label: "Query dependencies", run: () => toast.info("Sales → Date, Product, Region") },
    ],
    Tools: [
      { label: "Query diagnostics", run: () => toast.info("Diagnostics session started") },
      ...
      <div className="flex flex-col">
        ...
        <div className="grid grid-cols-[150px_1fr_210px]">
          <div className="border-r border-border p-2">
            <div className="mb-1 text-[11px] font-semibold">Queries</div>
            {TABLES.map((t) => (
              <button
                key={t.name}
                ...
                <tr>
                  {columns.map((c) => (
                    <th
                      ...
                      className="cursor-pointer whitespace-nowrap border-b border-r border-border px-2 py-1 text-left font-semibold hover:bg-accent"
                    >
                      {c}
                    </th>
                  ))}
              ...
              <tbody>
                {rows.map((r, i) => (
                  <tr key={i} className="hover:bg-accent">
                    {columns.map((c) => (
                      <td key={c} className="whitespace-nowrap border-b border-r border-border px-2 py-[3px]">
                        {typeof r[c] === "number" ? formatNumber(Number(r[c]), false) : String(r[c] ?? "")}
                      </td>
                    ))}
                  </tr>
                ))}
          ...
          <div className="border-l border-border p-2">
            <div className="mb-1 text-[11px] font-semibold">Applied steps</div>
            {steps.map((s, i) => (
              <div
                key={i}
        ...
        <div className="flex justify-end gap-2 border-t border-border bg-panel px-3 py-2">
          <button onClick={onClose} className="rounded border border-border px-3 py-1 text-[12px]">
            Cancel
          </button>
          <button
            onClick={() => {
...
export function DaxDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
    ...
    <Modal open={open} onClose={onClose} title="DAX formula bar">
      <div className="space-y-3 p-4">
        ...
        <div className="flex justify-end gap-2">
          ...
          <button
            onClick={() => {
              toast.success("Measure validated", { description: expr.split("=")[0]?.trim() });
              onClose();
            }}
            className="rounded bg-brand px-3 py-1 text-[12px] font-medium text-brand-foreground"
          >
...
export function PerformanceDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  const { page } = usePbi();
  return (
...
export function QnaDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const run = () => {
    ...
    setState((s) => ({
      ...s,
      pages: s.pages.map((p) =>
        p.id === activePageId ? { ...p, visuals: [...p.visuals, visual] } : p,
      ),
      selectedId: visual.id,
    }));
    toast.success("Q&A visual created", { description: `${measure} by ${by}` });
    onClose();
  };

  return (
    <Modal open={open} onClose={onClose} title="Q&A — ask a question about your data">
      <div className="space-y-3 p-4">
        <input
          value={q}
