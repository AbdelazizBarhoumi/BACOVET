...
function useInteractiveRows(visual: Visual, rows: Row[]) {
  ...
  return useMemo(() => {
    if (!crossFilter || crossFilter.sourceId === visual.id) return { rows, dim: false };
    const mode = interactionFor(crossFilter.sourceId, visual.id);
    if (mode === "none") return { rows, dim: false };
    const filtered = rows.filter((r) => String(r[crossFilter.column]) === crossFilter.value);
    return { rows: filtered, dim: mode === "highlight" };
  }, [crossFilter, interactionFor, rows, visual.id]);
}

export function VisualView({ visual, rows: allRows }: { visual: Visual; rows: Row[] }) {
  ...
  const onPointClick = (payload: { category?: string | number } | undefined) => {
  ...
  switch (visual.type) {
    case "text":
      return <div className="h-full w-full overflow-auto p-2 text-sm text-foreground">{visual.text}</div>;
    case "image":
      return visual.imageUrl ? (
        <img src={visual.imageUrl} alt={visual.altText || visual.name} className="h-full w-full object-contain" />
      ) : (
    ...
    case "smartNarrative":
      return <SmartNarrative visual={visual} rows={rows} data={data} series={series} />;
    case "keyInfluencers":
    ...
    case "filledMap":
    case "shapeMap":
      return <MapVisual data={data} series={series} onPointClick={onPointClick} />;
    default:
      break;
  }
  ...
  switch (visual.type) {
        ...
        <div className="flex h-full flex-wrap items-center justify-around gap-2">
          {visual.values.map((v, i) => (
            <div key={i} className="flex flex-col items-center">
              ...
              <div className="mt-1 text-[11px] text-muted-foreground">{measureLabel(v)}</div>
              {visual.axis[0] && (
                <div className="text-[10px] text-muted-foreground">by {visual.axis[0].name}</div>
              )}
            </div>
          ))}
        </div>,
    ...
    case "kpi": {
      ...
      return wrap(
        <div className="flex h-full flex-col items-center justify-center gap-1">
          <div className={cn("text-3xl font-semibold tracking-tight", good ? "text-success" : "text-destructive")}>
            {formatNumber(val)}
          </div>
          <div className="text-[11px] text-muted-foreground">{measureLabel(v)}</div>
          <div className="text-[10px] text-muted-foreground">Goal {formatNumber(goal)}</div>
        </div>,
      );
    }
    case "gauge": {
        ...
        <div className="relative h-full w-full">
          <ResponsiveContainer width="100%" height="100%">
            <RadialBarChart
              ...
              startAngle={180}
              endAngle={0}
              data={[{ name: "v", value: val, fill: "var(--chart-1)" }]}
            >
              <RadialBar background dataKey="value" cornerRadius={4} />
            </RadialBarChart>
          </ResponsiveContainer>
          <div className="pointer-events-none absolute inset-x-0 bottom-6 text-center">
            <div className="text-xl font-semibold text-foreground">{formatNumber(val)}</div>
            <div className="text-[10px] text-muted-foreground">of {formatNumber(max)}</div>
          </div>
        </div>,
      );
    }
        ...
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie
              ...
              outerRadius="85%"
              paddingAngle={1}
              onClick={(d: { category?: string }) => onPointClick(d)}
            >
              {data.map((_, i) => (
                <Cell key={i} fill={PALETTE[(i + visual.colorIndex) % PALETTE.length]} />
              ))}
              {visual.showLabels && (
                <LabelList dataKey={key} formatter={(x: number) => formatNumber(x)} style={{ fontSize: 9 }} />
              )}
            </Pie>
            <Tooltip contentStyle={tooltipStyle} formatter={(x: number) => formatNumber(x)} />
            {visual.showLegend && <Legend wrapperStyle={{ fontSize: 10 }} />}
          </PieChart>
        </ResponsiveContainer>,
      );
    }
    case "treemap": {
      ...
      return wrap(
        <ResponsiveContainer width="100%" height="100%">
          <Treemap data={tm} dataKey="size" nameKey="name" stroke="var(--card)" isAnimationActive={false}>
            <Tooltip contentStyle={tooltipStyle} formatter={(x: number) => formatNumber(x)} />
          </Treemap>
        </ResponsiveContainer>,
      );
    ...
    case "funnel":
      return wrap(
        <ResponsiveContainer width="100%" height="100%">
          <FunnelChart>
            <Tooltip contentStyle={tooltipStyle} formatter={(x: number) => formatNumber(x)} />
            <Funnel dataKey={series[0] ?? "value"} data={data} isAnimationActive>
              <LabelList position="right" dataKey="category" style={{ fontSize: 10, fill: "var(--foreground)" }} />
              {data.map((_, i) => (
    ...
    case "waterfall": {
      ...
      const wdata = data.map((d) => {
        ...
        const base = running;
        running += val;
        return { category: d["category"], base, delta: val, total: running };
      });
      return wrap(
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={wdata} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
            <CartesianGrid stroke="var(--border)" vertical={false} />
            <XAxis dataKey="category" {...axisProps} />
            <YAxis tickFormatter={(v) => formatNumber(v)} {...axisProps} />
            <Tooltip contentStyle={tooltipStyle} formatter={(x: number) => formatNumber(x)} />
          ...
          <AreaChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
            ...
            {series.map((s, i) => (
              <Area
                ...
                type="monotone"
                dataKey={s}
                {...(visual.type === "stackedArea" ? { stackId: "1" } : {})}
      ...
      return wrap(
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
            ...
            {series.map((s, i) =>
              i === 0 ? (
                <Bar key={s} dataKey={s} fill={PALETTE[0]} radius={[2, 2, 0, 0]} onClick={onPointClick} />
              ) : (
                <Line key={s} type="monotone" dataKey={s} stroke={PALETTE[i]} strokeWidth={2} />
              ),
            )}
            {analyticsLines(visual, data, series)}
          </ComposedChart>
      ...
      return wrap(
        <ResponsiveContainer width="100%" height="100%">
          <ScatterChart margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
            ...
            <XAxis dataKey={xKey} type="number" tickFormatter={(v) => formatNumber(v)} {...axisProps} />
            <YAxis dataKey={yKey} type="number" tickFormatter={(v) => formatNumber(v)} {...axisProps} />
            {visual.type === "bubble" && <ZAxis dataKey={zKey} range={[40, 500]} />}
            <Tooltip contentStyle={tooltipStyle} formatter={(x: number) => formatNumber(x)} />
            <Scatter data={data} fill="var(--chart-1)" onClick={onPointClick} />
          </ScatterChart>
        </ResponsiveContainer>,
      );
    }
    ...
    case "stackedBar":
    case "stacked100Bar": {
      const bdata = visual.type === "stacked100Bar" ? normalize(data, series) : data;
      const stacked = visual.type !== "bar" || visual.legend.length > 0;
      return wrap(
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={bdata} layout="vertical" margin={{ top: 8, right: 12, left: 8, bottom: 0 }}>
            <CartesianGrid stroke="var(--border)" horizontal={false} />
            <XAxis type="number" tickFormatter={(v) => formatNumber(v)} {...axisProps} />
            <YAxis type="category" dataKey="category" width={90} {...axisProps} />
            <Tooltip contentStyle={tooltipStyle} formatter={(x: number) => formatNumber(x)} />
            {visual.showLegend && series.length > 1 && <Legend wrapperStyle={{ fontSize: 10 }} />}
            {series.map((s, i) => (
              <Bar
                key={s}
                dataKey={s}
                {...(stacked ? { stackId: "a" } : {})}
                fill={PALETTE[(i + visual.colorIndex) % PALETTE.length]}
                radius={[0, 2, 2, 0]}
    ...
    default: {
      ...
      const cdata = visual.type === "stacked100Column" ? normalize(data, series) : data;
      const stacked =
      ...
      return wrap(
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={cdata} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
            <CartesianGrid stroke="var(--border)" vertical={false} />
            <XAxis dataKey="category" {...axisProps} />
            <YAxis tickFormatter={(v) => formatNumber(v)} {...axisProps} />
            <Tooltip contentStyle={tooltipStyle} formatter={(x: number) => formatNumber(x)} />
...
function normalize(data: Record<string, string | number>[], series: string[]) {
  return data.map((d) => {
    ...
    const out: Record<string, string | number> = { category: d["category"] as string };
    ...
    if (a.kind === "average")
      return (
        <ReferenceLine
          ...
          stroke="var(--chart-4)"
          strokeDasharray="4 4"
          label={{ value: `Average ${formatNumber(avg)}`, fontSize: 9, fill: "var(--muted-foreground)" }}
        />
      );
    if (a.kind === "constant")
      return (
        <ReferenceLine
          ...
          y={a.value ?? max * 0.8}
          stroke="var(--chart-5)"
          label={{ value: "Target", fontSize: 9, fill: "var(--muted-foreground)" }}
        />
      );
...
function TableVisual({ visual, rows }: { visual: Visual; rows: Row[] }) {
  ...
  const { data, series } = buildChartData(rows, visual.axis, visual.type === "matrix" ? visual.legend : [], visual.values);
  if (!groupCol && !visual.values.length) return <EmptyVisual label="Table" />;
  const maxByCol = Object.fromEntries(
    series.map((s) => [s, Math.max(...data.map((d) => Number(d[s]) || 0), 1)]),
  );
  const totals = series.map((s) => data.reduce((t, d) => t + (Number(d[s]) || 0), 0));
          ...
          <tr>
            {groupCol && (
              <th className="border-b border-border px-2 py-1 text-left font-semibold">
                {groupCol}
                {legendCol && visual.type === "matrix" ? ` / ${legendCol}` : ""}
              </th>
            )}
            {series.map((s) => (
              <th key={s} className="border-b border-border px-2 py-1 text-right font-semibold">
                {s}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {data.map((d, i) => (
            <tr key={i} className="hover:bg-accent">
              {groupCol && <td className="border-b border-border px-2 py-1">{d["category"]}</td>}
              {series.map((s) => {
                const val = Number(d[s]) || 0;
                const pct = (val / (maxByCol[s] as number)) * 100;
                return (
                  <td key={s} className="relative border-b border-border px-2 py-1 text-right tabular-nums">
                    ...
                    <span className="relative">{formatNumber(val)}</span>
                  </td>
                );
              })}
            </tr>
...
function SlicerVisual({ visual }: { visual: Visual }) {
  ...
  const values = distinctValues(col).filter((v) => v.toLowerCase().includes(q.toLowerCase()));
    ...
    return (
      <div className="flex h-full flex-col justify-center gap-2 p-2 text-[11px]">
        <label className="flex items-center justify-between gap-2">
          ...
          <input type="date" className="rounded border border-border bg-background px-2 py-1" />
        </label>
        <label className="flex items-center justify-between gap-2">
...
function SmartNarrative({
  ...
  const key = series[0];
  if (!key || !data.length)
    return <EmptyVisual label="Smart narrative — add a category and a measure" />;
  ...
  return (
    <div className="h-full overflow-auto p-2 text-[11px] leading-relaxed text-foreground">
      <p>
        <strong>{key}</strong> totalled <strong>{formatNumber(total)}</strong> across{" "}
        {data.length} {visual.axis[0]?.name ?? "categories"} and {rows.length.toLocaleString()} rows
        in the current filter context.
      </p>
      <p className="mt-2">
        <strong>{top["category"]}</strong> had the highest value at {formatNumber(Number(top[key]))} (
        {((Number(top[key]) / (total || 1)) * 100).toFixed(1)}% of total), while{" "}
        <strong>{bottom["category"]}</strong> was lowest at {formatNumber(Number(bottom[key]))}.
      </p>
...
function KeyInfluencers({ visual, rows }: { visual: Visual; rows: Row[] }) {
  ...
  if (!dim || !measure) return <EmptyVisual label="Key influencers — add a field and a measure" />;
  ...
  return (
    <div className="h-full overflow-auto p-1 text-[11px]">
      <p className="mb-2 text-muted-foreground">
        What influences <strong>{measureLabel(measure)}</strong> to increase?
      </p>
...
function DecompositionTree({ visual, rows }: { visual: Visual; rows: Row[] }) {
  ...
  const measure = visual.values[0];
  const levels = (visual.drillFields.length ? visual.drillFields : visual.axis).map((f) => f.name);
  if (!measure || !levels.length)
    return <EmptyVisual label="Decomposition tree — add Explain by fields and a measure" />;

  let scoped = rows;
  for (const p of path) scoped = scoped.filter((r) => String(r[p.col]) === p.val);
  ...
  return (
    <div className="flex h-full gap-3 overflow-auto p-1 text-[11px]">
      <div className="min-w-24">
        ...
        <div className="text-lg">{formatNumber(aggregate(scoped, measure))}</div>
        {path.map((p, i) => (
          <button
            key={i}
      ...
      {nextCol && (
        <div className="min-w-40 flex-1">
          <div className="mb-1 font-semibold text-muted-foreground">{nextCol}</div>
          {items.slice(0, 12).map((it) => (
            <button
              key={it.k}
              onClick={() => setPath([...path, { col: nextCol, val: it.k }])}
              className="mb-0.5 block w-full rounded px-1 text-left hover:bg-accent"
            >
              <span className="flex justify-between">
                <span className="truncate">{it.k}</span>
                <span className="tabular-nums">{formatNumber(it.v)}</span>
              </span>
...
function QnaVisual({ visual, rows }: { visual: Visual; rows: Row[] }) {
  ...
  const col = /country/i.test(q) ? "Country" : /month/i.test(q) ? "Month" : "Category";
  const { data, series } = buildChartData(
    rows,
    [{ table: "Sales", name: col, agg: "count" }],
    [],
    visual.values.length ? visual.values : [{ table: "Measures", name: "Total Sales", agg: "sum" }],
  );
  return (
      ...
      <div className="min-h-0 flex-1">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data}>
            ...
            <YAxis tickFormatter={(v) => formatNumber(v)} {...axisProps} />
            <Tooltip contentStyle={tooltipStyle} formatter={(x: number) => formatNumber(x)} />
            <Bar dataKey={series[0] ?? "value"} fill="var(--chart-1)" radius={[2, 2, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
      ...
      <div className="min-h-0 flex-1">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data}>
            ...
            <XAxis dataKey="category" {...axisProps} />
            <YAxis tickFormatter={(v) => formatNumber(v)} {...axisProps} />
            <Line type="monotone" dataKey={series[0] ?? "value"} stroke="var(--chart-3)" dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
      ...
      {data.map((d, i) => {
        ...
        return (
          <button
            ...
            style={{ backgroundColor: `color-mix(in oklch, var(--chart-1) ${(v / max) * 80 + 10}%, transparent)` }}
          >
            <span className="truncate">{d["category"]}</span>
            <span className="font-semibold tabular-nums">{formatNumber(v)}</span>
          </button>
        );
      })}
    </div>
