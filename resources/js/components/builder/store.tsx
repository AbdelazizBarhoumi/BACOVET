...
const DEFAULT_CONFIG_FOR: Record<WidgetType, WidgetConfig> = {
  kpi: { ...STYLE_DEFAULTS, label: "KPI", mockValue: 92.5, unit: "%", decimals: 1, target: 90, accent: "#22c55e", showTarget: true, showLabel: true, shadow: "sm" },
  gauge: { ...STYLE_DEFAULTS, label: "Gauge", mockValue: 89, target: 85, accent: "#3b82f6", shadow: "sm" },
  sparkline: { ...STYLE_DEFAULTS, label: "Trend", mockSeries: [1,2,3,4,3,5,6,7].map((v,i)=>({x:String(i),v})), accent: "#3b82f6" },
  line: { ...STYLE_DEFAULTS, label: "Line chart", mockSeries: [10,20,15,30,25,40,35].map((v,i)=>({x:`T${i}`,v})), accent: "#3b82f6", target: 30, showTarget: true },
  bar: { ...STYLE_DEFAULTS, label: "Bar chart", mockSeries: [10,20,15,30,25,40].map((v,i)=>({x:`C${i}`,v})), accent: "#ec4899" },
  pareto: { ...STYLE_DEFAULTS, label: "Pareto", mockSeries: [720,480,360,260,180,120].map((v,i)=>({x:`Def ${i+1}`,v})) },
  donut: { ...STYLE_DEFAULTS, label: "Donut", mockValue: 76, accent: "#22c55e" },
  pie: { ...STYLE_DEFAULTS, label: "Pie Chart", mockSeries: [{x:"A",v:35},{x:"B",v:25},{x:"C",v:20},{x:"D",v:15},{x:"E",v:5}] },
  radar: { ...STYLE_DEFAULTS, label: "Radar", mockSeries: [{x:"Speed",v:80},{x:"Power",v:65},{x:"Defense",v:90},{x:"Agility",v:70},{x:"Stamina",v:85}] },
  area: { ...STYLE_DEFAULTS, label: "Area Chart", mockSeries: [10,20,15,30,25,40,35,50,45,55].map((v,i)=>({x:`T${i}`,v})), accent: "#3b82f6", target: 30, showTarget: true },
  combo: { ...STYLE_DEFAULTS, label: "Combo Chart", mockSeries: [10,20,15,30,25,40].map((v,i)=>({x:`C${i}`,v})), accent: "#3b82f6", target: 25, showTarget: true },
