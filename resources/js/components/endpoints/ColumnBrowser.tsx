...
export function ColumnBrowser({
    ...
    const active = useMemo(
        () =>
            filtered.find((column) => column.name === selected) ??
            filtered[0] ??
            null,
        [filtered, selected],
    );
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)]">
            <div className="space-y-2">
                <div className="relative">
                    ...
                    <Input
                        ...
                        placeholder="Rechercher une colonne, ex. ProdGroup…"
            ...
            {active ? (
                <div className="space-y-3">
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="font-mono text-[10px] tracking-wider text-muted-foreground uppercase">
                            {active.endpoint_count} endpoint
                            {active.endpoint_count > 1 ? 's' : ''}
                        </span>
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                        <div className="rounded-md border border-border">
                            <div className="border-b border-border px-3 py-2 font-mono text-[10px] tracking-wider text-muted-foreground uppercase">
                                Endpoints contenant cette colonne
                            </div>
                            <div className="max-h-64 overflow-auto">
                                {active.endpoints.map((endpoint) => (
                                    ...
                                    >
                                        ...
                                        <span className="shrink-0 font-mono text-[10px] text-muted-foreground">
                                            {endpoint.distinct_count} distinctes
                                        </span>
                                    </button>
                                ))}
                            </div>
                        ...
                        <div className="rounded-md border border-border">
                            <div className="border-b border-border px-3 py-2 font-mono text-[10px] tracking-wider text-muted-foreground uppercase">
                                Valeurs d'exemple (
                                {active.distinct_values.length} affichées)
                            </div>
                ...
                <div className="rounded-md border border-dashed border-border p-8 text-center text-xs text-muted-foreground">
                    Sélectionnez une colonne pour inspecter quels endpoints la
                    partagent et quelles valeurs existent.
                </div>
            )}
        </div>
    );
