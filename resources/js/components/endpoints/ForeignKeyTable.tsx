    ...
    if (foreignKeys.length === 0) {
        return (
            <div className="rounded-md border border-dashed border-border p-6 text-center text-xs text-muted-foreground">
                No foreign-key candidates found — try widening the sample data.
            </div>
        );
    }
            ...
            <table className="w-full text-sm">
                <thead className="sticky top-0 bg-background">
                    <tr className="border-b border-border font-mono text-[10px] tracking-wider text-muted-foreground uppercase">
                        <th className="py-2 px-3 text-left">Source endpoint</th>
                        <th className="py-2 px-3 text-left">Column</th>
                        <th className="py-2 px-3 text-left">References</th>
                            ...
                            reference.refs.map((ref) => (
                                ...
                                >
                                    ...
                                    >
                                        <button
                                            type="button"
                                            onClick={() => onOpenEntry(fk.entry_id)}
                                            className="cursor-pointer hover:underline"
                                        >
                                            {fk.entry_name}
                                        </button>
                                    </td>
                                    <td className="px-3 text-xs font-bold">{reference.column}</td>
                                    <td className="max-w-[260px] truncate px-3 text-xs" title={ref.entry_name}>
                                        <button
                                            type="button"
                                            onClick={() => onOpenEntry(ref.entry_id)}
                                            className="cursor-pointer hover:underline"
                                        >
