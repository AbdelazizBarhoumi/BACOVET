    ...
    return (
        <div className="space-y-3">
            <div className="flex items-center gap-3 font-mono text-[10px] tracking-wider text-muted-foreground uppercase">
                <span>
                    {rowCount} lignes · {columns.length} colonnes
                </span>
                {hasData ? (
                    <span className="text-success">avec données</span>
                ) : (
                    <span className="text-warning">aucune ligne d'exemple</span>
                )}
            </div>
            {columns.length === 0 ? (
                <div className="rounded-md border border-dashed border-border p-6 text-center text-xs text-muted-foreground">
                    Aucune colonne n'a pu être déduite — la réponse ne contient
                    ni métadonnées
                    <code className="mx-1 font-mono">columns</code>
                    ni lignes de données.
                </div>
            ) : (
                    ...
                    <table className="w-full text-sm">
                        <thead className="sticky top-0 bg-background">
                            <tr className="border-b border-border font-mono text-[10px] tracking-wider text-muted-foreground uppercase">
                                ...
                                <th className="px-3 py-2 text-left">Exemple</th>
                        ...
                        <tbody className="font-mono">
                            {columns.map((col) => (
                                ...
                                >
                                    <td className="px-3 py-1.5 text-xs font-semibold break-all">
                                        {col.name}
                                    </td>
                                    <td className="px-3">
                                    ...
                                    <td
                                        className="max-w-[260px] truncate px-3 text-xs text-muted-foreground"
                                        title={sampleText(col.sample)}
                                    >
