...
export function FieldsPane({ onCollapse }: { onCollapse?: () => void }) {
    ...
    // Keep the tables used by the selected visual expanded. This is a derived
    // adjustment performed during render (not in an effect) so it doesn't
    // trigger a cascading re-render after commit.
    const [expandedFor, setExpandedFor] = useState(selected);
    if (selected !== expandedFor) {
        ...
        if (selected) {
            ...
            const wells = [
                ...
                selected.legend,
                selected.tooltips,
                selected.smallMultiples,
                selected.drillFields,
                selected.minimum,
                selected.maximum,
                selected.target,
            ];
            for (const well of wells) {
                for (const f of well ?? []) {
                    if (f.table && f.table !== 'Measures') used.add(f.table);
                }
            }
            if (used.size) {
                setOpen((o) => {
