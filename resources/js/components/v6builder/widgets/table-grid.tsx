...
export function TableGridWidget({ c, editing, onCellSelect, selectedCells, cursor,
...
function TableGridRenderer({ t, editing, onCellSelect, selectedCells, cursor,
  onCopy, onPaste, onInsertRow, onInsertCol, onDeleteRow, onDeleteCol, onResize }: {
  t: TableGrid;
  editing: boolean;
  onCellSelect?: (r: number, c: number, add: boolean) => void;
  ...
  for (let r = 0; r < t.rows; r++) {
    for (let c = 0; c < t.cols; c++) {
      ...
      cells.push(
        <div
          key={`${r},${c}`}
          className={`no-drag overflow-hidden text-xs px-2 py-1 flex items-center border border-border transition-colors ${selected ? "outline outline-2 outline-primary z-10" : ""} ${active ? "ring-2 ring-blue-500" : ""} ${editing ? "hover:bg-accent/40" : ""}`}
          style={{
  ...
  return (
    <div ref={gridRef} className="h-full w-full grid relative" style={{ gridTemplateColumns: colTemplate, gridTemplateRows: rowTemplate }}>
      ...
      {ctxMenu && (
        <ContextMenu ref={ctxRef} x={ctxMenu.x} y={ctxMenu.y} items={ctxItems} />
      )}
    </div>
  );
}

const ContextMenu = ({ ref, x, y, items }: {
