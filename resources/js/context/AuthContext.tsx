...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const cameFromLogin = previousUrl.current === "/login" && page.url !== "/login";
  ...
  const hasAccess = useMemo(() => (page: RolePage) => {
  ...
  const login = useMemo(() => async (matricule: string, pwd: string) => {
