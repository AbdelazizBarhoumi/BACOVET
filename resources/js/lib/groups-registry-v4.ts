...
import type { BuilderPageV4 } from "./pages-registry-v4";

export type BuilderPageGroupV4 = {
  id: number;
  name: string;
  slug: string;
  sort_order: number;
...
export type SidebarStructureV4 = {
  groups: BuilderPageGroupV4[];
  ungrouped: BuilderPageV4[];
};

const API_BASE = "/api/v4/builder-page-groups";
...
async function fetchSidebar(): Promise<SidebarStructureV4> {
  try {
...
async function apiCreateGroup(name: string): Promise<BuilderPageGroupV4 | null> {
  try {
    const res = await fetch(API_BASE, {
      method: "POST",
      credentials: "include",
...
export function useSidebarStructureV4() {
  const [data, setData] = useState<SidebarStructureV4>({ groups: [], ungrouped: [] });
  ...
  const createGroup = useCallback(async (name: string): Promise<BuilderPageGroupV4 | null> => {
