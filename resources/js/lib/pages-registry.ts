...
export type AccessMode = 'view' | 'edit' | 'none';
...
const API_BASE = '/api/builder-pages';
...
async function apiDuplicatePage(id: number): Promise<BuilderPage | null> {
    try {
...
export async function savePagePermissions(
    id: number,
    permissions: { user_id: number; mode: AccessMode }[],
): Promise<boolean> {
    try {
        const res = await fetch(`${API_BASE}/${id}/permissions`, {
            method: 'PUT',
