...
describe('activeSelection', () => {
    it('ignores parameters of other pages', () => {
        const parameters = [
            param({ id: 'a', pageId: 'page-a', name: 'chaine', value: 'CH02' }),
    ...
    it('skips null / empty values', () => {
        const parameters = [
            param({ id: 'a', name: 'chaine', value: null }),
            param({ id: 'b', name: 'zone', value: '' }),
            param({ id: 'c', name: 'autre', value: 'x' }),
        ];
        expect(activeSelection(parameters, 'page-a')).toEqual({ autre: 'x' });
    });
});
...
describe('datasetsUrlWithSelection', () => {
    ...
    it('appends each parameter as p[name]=value', () => {
        ...
        expect(url).toBe('/api/endpoint-datasets?p%5Bchaine%5D=CH02');
    });
    ...
    it('supports several parameters and a custom base URL', () => {
        ...
        expect(url).toContain('p%5Bchaine%5D=CH02');
        expect(url).toContain('p%5Bzone%5D=ZA');
    });
});
