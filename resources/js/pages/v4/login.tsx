...
export default function V4LoginPage() {
  ...
  const handleSelectEmail = async (email: string, name: string) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/v4-auth/check", {
        method: "POST",
  ...
  const handleCreatePassword = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/v4-auth/set-password", {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json", "X-Requested-With": "XMLHttpRequest", "X-XSRF-TOKEN": getCsrfToken() },
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <Head title="Connexion V4 — BACOVET" />
      <div className="w-full max-w-sm">
        <div className="bg-card border border-border rounded-lg shadow-lg p-6">
          <div className="flex items-center gap-2 mb-6">
            <Lock className="h-5 w-5 text-primary" />
            <h1 className="text-lg font-semibold">Pages Builder V4</h1>
          </div>
