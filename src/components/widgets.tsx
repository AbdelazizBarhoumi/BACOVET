...
export function TrafficBadge({ status, children }: { status: Status; children?: ReactNode }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border px-2 py-0.5 text-[10px] font-mono uppercase tracking-wider ${map[status]}`}>
      <span className={`h-1.5 w-1.5 rounded-full ${status === "green" ? "bg-success" : status === "orange" ? "bg-warning" : "bg-destructive"}`} />
...
export function BigNumberCard({
  label, value, unit = "%", target, status, source, trend, freq,
}: {
  label: string; value: number | string; unit?: string;
  target?: string; status?: Status; source?: string;
  trend?: "up" | "down" | "flat"; freq?: string;
}) {
  const bar = status === "green" ? "bg-success" : status === "orange" ? "bg-warning" : status === "red" ? "bg-destructive" : "bg-muted";
  return (
    <div className="relative overflow-hidden rounded-lg border border-border bg-card p-4">
      ...
      {source && <div className="mt-1 text-[10px] font-mono text-muted-foreground/70 truncate">src: {source}</div>}
    </div>
  );
}
  ...
  return (
    <div className={`rounded-lg border border-border bg-card ${className}`}>
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
        <h2 className="text-xs font-bold uppercase tracking-[0.15em] text-foreground/90">{title}</h2>
        {right}
      </div>
