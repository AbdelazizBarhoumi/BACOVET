...
function loadUsers(): User[] {
  ...
  return adminData.users.map((u, i) => ({
    id: `u${i}`, name: u.name, role: "resp_production" as Role, email: u.email, active: u.active,
  }));
}
...
function saveScreens(s: Screen[]) { localStorage.setItem(SCREENS_KEY, JSON.stringify(s)); }
...
function AdminPage() {
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener("bacovet-audit", refresh); clearInterval(id); };
  }, []);
    ...
    <AppShell page="/admin" title="Administration" subtitle="Panneau de contrôle système">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        <div className="lg:col-span-2 space-y-3">
            ...
            <div className="flex flex-wrap items-end gap-4">
              <div>
                ...
                <Input type="number" min={10} max={600} value={refreshIntervalSec}
                  onChange={e => setRefreshIntervalSec(Math.max(10, Math.min(600, Number(e.target.value) || 60)))}
                  className="w-32 font-mono" />
              </div>
              <Button size="sm" onClick={() => { forceSync(); pushAudit("SYSTEM", "Sync globale forcée"); }}>
                Forcer la synchronisation
              </Button>
              <div className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground ml-auto">
                Dernière sync: <span className="text-foreground">{new Date(lastSync).toLocaleTimeString()}</span>
              </div>
            </div>
          </Panel>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-[10px] uppercase tracking-wider text-muted-foreground font-mono border-b border-border">
                  <th className="text-left py-2">Source</th><th className="text-left">Statut</th>
                  <th className="text-left">Dernière sync</th><th className="text-right">Action</th>
                </tr>
              </thead>
                  ...
                  <tr key={a.name} className="border-b border-border/50">
                    ...
                    <td className="text-right">
                      <Button size="sm" variant="outline"
                        onClick={() => { forceSync(); pushAudit("SYSTEM", `Job forcé: ${a.name}`); }}
          ...
          <Panel title="Gestion des comptes" right={
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-[10px] uppercase tracking-wider text-muted-foreground font-mono border-b border-border">
                  <th className="text-left py-2">Utilisateur</th><th className="text-left">Rôle</th>
                  <th className="text-left">Email</th><th className="text-left">Statut</th><th className="text-right">Action</th>
                </tr>
              </thead>
              <tbody className="font-mono">
                {users.map(u => (
                  <tr key={u.id} className="border-b border-border/50">
                    ...
                    <td className="text-right">
                      <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => setEditing(u)}>
                        <Pencil className="h-3 w-3" />
                      </Button>
                      <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-destructive" onClick={() => deleteUser(u.id)}>
          ...
          <Panel title="Journal d'audit système" right={
            <div className="flex items-center gap-2 text-[10px] font-mono uppercase tracking-wider">
              Enregistrement actif
              <Switch checked={auditOn} onCheckedChange={(v) => { setAuditOn(v); setAuditEnabled(v); pushAudit("SYSTEM", `Audit ${v ? "activé" : "désactivé"}`); }} />
            </div>
          }>
            <div className="space-y-1 font-mono text-xs max-h-80 overflow-auto">
              {logs.length === 0 && <div className="text-muted-foreground italic">Aucun événement enregistré.</div>}
              {logs.map((l, i) => {
                const color = l.lvl === "ERROR" ? "text-destructive" : l.lvl === "WARN" ? "text-warning" :
                  l.lvl === "USER" ? "text-chart-4" : l.lvl === "SYSTEM" ? "text-primary" : "text-success";
                return (
        ...
        <div className="space-y-3">
          <Panel title="Écrans TV">
            <div className="space-y-2">
              {screens.map((s, i) => (
                <div key={s.name} className="rounded-md border border-border bg-secondary/40 p-3">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      onClick={() => {
                        ...
                        setScreens(next);
                        pushAudit("SYSTEM", `Écran ${s.name}: ${!s.online ? "en ligne" : "hors ligne"}`);
                      }}
                  ...
                  <Select value={s.view} onValueChange={(v) => {
                    const next = [...screens];
                    next[i] = { ...s, view: v };
                    setScreens(next);
                    ...
                    <SelectContent>
                      {[
                        ...
                        "Logistique (300)",
                        "Développement (350)",
                      ].map(v => <SelectItem key={v} value={v} className="text-[10px] font-mono">{v}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
              ))}
            ...
            <div className="mt-3 text-[10px] font-mono uppercase tracking-wider text-muted-foreground">
              ...
              {" · "}
              En ligne : <span className="text-success">{screens.filter(s => s.online).length}</span>
            </div>
          </Panel>
        </div>
      </div>
          ...
          <Select value={role} onValueChange={(v) => setRole(v as Role)}>
            ...
            <SelectContent>
              {(Object.keys(ROLE_LABEL) as Role[]).map(r => (
                <SelectItem key={r} value={r} className="font-mono text-xs">{ROLE_LABEL[r]}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
