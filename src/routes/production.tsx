...
export const Route = createFileRoute("/production")({
  head: () => ({ meta: [{ title: "Production — BACOVET" }] }),
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
        <BigNumberCard label="Efficience Chaîne · F-REQ-202" value={avgEff} target="> 85%" status={statusFor(avgEff, 85)} source="q/efficience_chaine" />
        <BigNumberCard label="OWE Chaîne · F-REQ-204" value={avgOwe} target="> 70%" status={statusFor(avgOwe, 70)} source="q/minutes_*" />
        <BigNumberCard label="WIP Chaîne · F-REQ-205" value={totalWip} unit="pc" target="≤ ½ cadence" status={totalWip < 1200 ? "green" : "orange"} source="q/wip_chaine" />
        <BigNumberCard label="Arrêts non planifiés · F-REQ-207" value={totalArrets} unit="min" target="< 10 min" status={statusFor(totalArrets, 10, "max")} source="q/lost_time" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-4">
        <Panel title="Efficience par Chaîne · Gauges">
          <div className="flex justify-around items-end pt-2">
            {p.chains.map(c => <Gauge key={c.id} value={c.eff} label={c.id} />)}
          </div>
        </Panel>
        ...
        <Panel title="Chronologie des arrêts (S1 + S2)">
          <div className="space-y-3 font-mono text-xs">
            {["CH1", "CH2", "CH3"].map(ch => (
              <div key={ch} className="flex items-center gap-2">
                ...
                <div className="relative flex-1 h-6 rounded bg-secondary overflow-hidden">
                  {p.stoppages.filter(s => s.chaine === ch).map((s, i) => {
                    const left = ((s.start - 6) / 12) * 100;
                    const width = (s.duration / 12) * 100;
                    const color = s.motif === "MAINT" ? "var(--chart-4)" : s.motif === "MATIERE" ? "var(--warning)" : "var(--destructive)";
                    return (
                      <div key={i} className="absolute top-0 h-full rounded text-[10px] flex items-center justify-center text-background font-bold"
            ...
            <div className="flex justify-between text-[10px] text-muted-foreground pl-12">
              <span>06h</span><span>09h</span><span>12h</span><span>15h</span><span>18h</span>
            </div>
            ...
            {p.ofProgress.map(o => (
              <div key={o.of} className="flex flex-col items-center">
                <ResponsiveContainer width="100%" height={120}>
                  <PieChart>
                    <Pie data={[{ v: o.pct }, { v: 100 - o.pct }]} dataKey="v" innerRadius={32} outerRadius={48} startAngle={90} endAngle={-270}>
        ...
        <Panel title="SO Progress par OF · F-REQ-304">
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={p.soByOf} layout="vertical">
              ...
              <YAxis dataKey="of" type="category" width={80} tick={{ fill: "var(--muted-foreground)", fontSize: 11 }} />
        ...
        <Panel title="Top Opérateurs · F-REQ-210">
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={p.topOperators} layout="vertical">
              <CartesianGrid stroke="var(--border)" strokeDasharray="3 3" />
              <XAxis type="number" unit="%" tick={{ fill: "var(--muted-foreground)", fontSize: 11 }} />
              <YAxis dataKey="nom" type="category" width={90} tick={{ fill: "var(--muted-foreground)", fontSize: 11 }} />
              <Tooltip contentStyle={tt} />
        ...
        <Panel title="WIP Optimal (7 derniers jours) · F-REQ-206" className="mb-4">
          <ResponsiveContainer width="100%" height={260}>
            <AreaChart data={p.wip7d}>
              ...
              <Tooltip contentStyle={tt} />
              <Legend wrapperStyle={{ fontSize: 11 }} />
              <Area type="monotone" dataKey="sortie" name="Sortie coupe" stroke="var(--success)" fill="var(--success)" fillOpacity={0.25} />
              <Area type="monotone" dataKey="engagement" name="Engagement" stroke="var(--warning)" fill="var(--warning)" fillOpacity={0.25} />
            </AreaChart>
          </ResponsiveContainer>
        </Panel>
      )}
      ...
      <Panel title="Efficience Cumulée · F-REQ-203">
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={production.effCumul}>
            ...
            <YAxis domain={[60, 120]} unit="%" tick={{ fill: "var(--muted-foreground)", fontSize: 11 }} />
            <Tooltip contentStyle={tt} />
            <ReferenceLine y={85} stroke="var(--success)" strokeDasharray="4 4" />
            <Line type="monotone" dataKey="eff" stroke="var(--primary)" strokeWidth={2} dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </Panel>
    </>
...
function ProductionPage() {
  const exportRows = production.chains.map(c => ({
    chaine: c.id, of: c.of, article: c.article, sam: c.sam, effectif: c.effectif,
    objectif: c.objectif, efficience_pct: c.eff, owe_pct: c.owe, wip: c.wip, arrets_min: c.arrets,
  }));
  return (
    <AppShell page="/production" title="Production & Flux" subtitle="Série 200 · Performance Atelier"
      exportRows={exportRows} exportFilename="BACOVET_Production_S200">

      <Tabs defaultValue="confection">
        <TabsList>
          <TabsTrigger value="confection" className="text-xs uppercase tracking-wider">Confection</TabsTrigger>
          <TabsTrigger value="coupe" className="text-xs uppercase tracking-wider">Coupe</TabsTrigger>
          <TabsTrigger value="serigraphie" className="text-xs uppercase tracking-wider">Sérigraphie</TabsTrigger>
        </TabsList>
        <TabsContent value="confection"><ProductionTab workshop="confection" /></TabsContent>
        <TabsContent value="coupe"><ProductionTab workshop="coupe" /></TabsContent>
        <TabsContent value="serigraphie"><ProductionTab workshop="serigraphie" /></TabsContent>
      </Tabs>
    </AppShell>
  );
}
