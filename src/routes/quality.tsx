...
const tooltipStyle = { backgroundColor: "var(--card)", border: "1px solid var(--border)", fontSize: 12 };

function colorFor(v: number, target: number, kind: "min" | "max") {
  const s = statusFor(v, target, kind);
  return s === "green" ? "var(--success)" : s === "orange" ? "var(--warning)" : "var(--destructive)";
}
...
function generateAlerts(q: typeof quality): Alert[] {
  ...
  push(q.brBundlingToday > 4 && q.brBundlingToday <= 5, "BR BUNDLING VIGILANCE", "OF-courant", "orange");
  push(q.brPrintToday > 5, "BR PRINT CRITIQUE", "Sérigraphie", "red");
  push(q.brCglYear > 5, "BR CGL ANNÉE DÉPASSÉ", "DDA", "red");
  // Stage-level
  q.brByStage.forEach(s => push(s.value > 5, `BR ${s.stage.toUpperCase()} DÉPASSÉ`, s.stage, "red"));
...
function QualityPage() {
  ...
  return (
    <AppShell page="/quality" title="Qualité" subtitle="Série 100 · Performance Qualité"
      exportRows={exportRows} exportFilename="BACOVET_Qualite_S100">

      {/* Section A — Big KPIs */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
        <BigNumberCard label="RFT (Ce jour) · F-REQ-104" value={q.rftToday} target="≥ 98%" status={statusFor(q.rftToday, 98)} source="q/pieces_ok_premier_coup" />
        <BigNumberCard label="BR Bundling (Ce jour) · F-REQ-106" value={q.brBundlingToday} target="≤ 5%" status={statusFor(q.brBundlingToday, 5, "max")} source="q/rejets_inspection_paquet" />
        <BigNumberCard label="BR Print (Ce jour) · F-REQ-108" value={q.brPrintToday} target="≤ 5%" status={statusFor(q.brPrintToday, 5, "max")} source="Google Drive" />
        <BigNumberCard label="BR CGL (Année) · F-REQ-101" value={q.brCglYear} target="≤ 5%" status={statusFor(q.brCglYear, 5, "max")} source="DIVA database" />
      </div>
        ...
        <Panel title="Bad Rate par étape de contrôle" className="lg:col-span-2">
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={q.brByStage}>
              ...
              <ReferenceLine y={5} stroke="var(--warning)" strokeDasharray="4 4" label={{ value: "Cible 5%", fill: "var(--warning)", fontSize: 10 }} />
              <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                {q.brByStage.map((d, i) => <Cell key={i} fill={colorFor(d.value, 5, "max")} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </Panel>
        ...
        <Panel title="Dernières alertes qualité">
          <div className="space-y-2">
            {generateAlerts(q).map((a, i) => (
              <div key={i} className="flex items-center justify-between rounded-md border border-border bg-secondary/40 px-3 py-2">
                <div className="flex items-center gap-2 min-w-0">
                  <TrafficBadge status={a.level} />
                  <div className="min-w-0">
                    <div className="text-xs font-bold uppercase tracking-wider truncate">{a.type}</div>
      ...
      <Panel title="RFT & BR — Année en cours" className="mb-4">
        <ResponsiveContainer width="100%" height={280}>
          <LineChart data={q.trend}>
            ...
            <Line type="monotone" dataKey="rft" name="RFT DDA" stroke="var(--chart-4)" strokeWidth={2} dot={{ r: 3 }} />
            <Line type="monotone" dataKey="br" name="BR Bundling DDA" stroke="var(--destructive)" strokeWidth={2} dot={{ r: 3 }} />
          </LineChart>
        </ResponsiveContainer>
      </Panel>
      ...
      <Panel title="Pareto des défauts">
        <Tabs defaultValue="rft">
          <TabsList>
            <TabsTrigger value="rft" className="text-xs uppercase tracking-wider">Pareto RFT (F-REQ-116)</TabsTrigger>
            <TabsTrigger value="colis" className="text-xs uppercase tracking-wider">Pareto Inspection Colis (F-REQ-117)</TabsTrigger>
          </TabsList>
          <TabsContent value="rft">
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={q.paretoRft} layout="vertical">
                ...
                <YAxis dataKey="op" type="category" width={130} tick={{ fill: "var(--muted-foreground)", fontSize: 11 }} />
                <Tooltip contentStyle={tooltipStyle} />
          ...
          <TabsContent value="colis">
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={q.paretoColis} layout="vertical">
                <CartesianGrid stroke="var(--border)" strokeDasharray="3 3" />
                <XAxis type="number" tick={{ fill: "var(--muted-foreground)", fontSize: 11 }} />
                <YAxis dataKey="item" type="category" width={130} tick={{ fill: "var(--muted-foreground)", fontSize: 11 }} />
                <Tooltip contentStyle={tooltipStyle} />
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-[10px] uppercase tracking-wider text-muted-foreground font-mono border-b border-border">
              <th className="text-left py-2">KPI ID</th><th className="text-left">Indicateur</th>
              <th className="text-right">Valeur</th><th className="text-right">Cible</th><th className="text-right">Statut</th>
            </tr>
          </thead>
          <tbody className="font-mono">
            {[
              ...
              ["F-REQ-106", "BR Bundling (jour)", q.brBundlingToday, "≤ 5%", statusFor(q.brBundlingToday, 5, "max")],
              ["F-REQ-107", "BR Bundling DDA", q.brBundlingYear, "≤ 5%", statusFor(q.brBundlingYear, 5, "max")],
            ].map((r, i) => (
              <tr key={i} className="border-b border-border/50">
                ...
                <td className="text-right"><TrafficBadge status={r[4] as "green" | "orange" | "red"} /></td>
              </tr>
            ))}
          </tbody>
        </table>
